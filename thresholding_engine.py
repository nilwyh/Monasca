                ...
                if self._publish_kafka_conn:
                    if lock.acquire():
                        for processor in self.thresholding_processors:
                            alarms.extend(
                                self.thresholding_processors[processor]
                                    .process_alarms())
                    lock.release()
                    for alarm in alarms:
                        self._publish_kafka_conn.send_messages(alarm)
			LOG.debug('Alarm created!!!')
			LOG.debug(alarm)
                time.sleep(self.interval)
            except Exception:
                LOG.exception(
                    'Error occurred while publishing kafka messages.')
...
class ThresholdingEngine(os_service.Service):
    ...
    def start(self):
        try:
            ...
            LOG.info('Starting thresh process succeed!')
        except Exception:
            LOG.info('Starting thresh process failed!')
